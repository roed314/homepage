...
def span_title(cell):
    """Return (text, title-attr) for a cell that may be a <span title=...>."""
    m = re.search(r'<span title="([^"]*)">(.*?)</span>', cell, re.S)
    if m:
        return re.sub(r"<[^>]+>", "", m.group(2)).strip(), m.group(1)
    return re.sub(r"<[^>]+>", "", cell).strip(), ""


def gen_teaching(cv):
    sec = cv_section(cv, "Teaching Experience")
    entries = []
    for number, course, institution, role, when in sec["rows"]:
        number_text = re.sub(r"<[^>]+>", "", number).strip()
        course_text, course_note = span_title(course)
        role_text, role_note = span_title(role)
        heading = f"{escape(role_text)}: {mathify(course_text)}"
        if number_text:
            heading += f" ({escape(number_text)})"
        note = "; ".join(n for n in (role_note, course_note) if n)
        entries.append((escape(when), heading, html2latex(institution), mathify(note)))
    when, heading, inst, note = entries[0]
    lines = [r"\goodindententrysection{\textsc{Teaching Experience}}{%s}{%s}{%s}{}{}{%s}"
             % (when, heading, inst, note), ""]
    for when, heading, inst, note in entries[1:]:
        lines.append(r"\cventry{%s}{%s}{%s}{}{}{%s}" % (when, heading, inst, note) + "\n")
    return "\n".join(lines)
